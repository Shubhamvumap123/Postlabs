...
export default function Footer() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // SECURITY: Client-side fetch to Mailchimp with API keys is a critical vulnerability.
    // API keys must be kept on the backend. This simulates the subscription in Demo Mode.
    try {
      await new Promise((resolve) => setTimeout(resolve, 800)); // Simulate network request
      alert(`Demo Mode: Thanks for signing up, ${email}! (No real API call made)`);
      form.reset();
    } catch (error) {
      console.error(error);
