...
export default function FullWidthVideoSection() {
  ...
  // Start loading/playing when within 200px of viewport
  const isPlaybackInView = useInView(containerRef, { margin: "0px 0px 200px 0px" });

  useEffect(() => {
    if (isPlaybackInView) {
      // Play returns a promise, which we catch to avoid errors if play is interrupted
