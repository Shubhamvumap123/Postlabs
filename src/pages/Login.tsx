...
const Login = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error instanceof Error && 'response' in error) { toast.error((error as { response?: { data?: { message?: string } } }).response?.data?.message || 'Login failed'); } else { toast.error('Login failed'); }
    }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-zinc-950 p-4">
      <div className="bg-zinc-900 border border-zinc-800 p-8 rounded-xl w-full max-w-md">
        <h2 className="text-2xl font-semibold text-white mb-6 text-center">Login to JobTracker</h2>
