...
const JobDashboard = () => {
  ...
  const filteredJobs = useMemo(() => {
    // PERFORMANCE: Hoist searchTerm string manipulation outside the loop to prevent O(N) redundant allocations
    const normalizedSearch = searchTerm.toLowerCase();
    return jobs.filter((job) => {
      const matchesSearch = job.company.toLowerCase().includes(normalizedSearch) ||
