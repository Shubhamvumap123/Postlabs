...
## 2026-06-30 - Add input length limits to text fields
**Vulnerability:** Missing input length limits on user-facing forms (Contact, Task Dashboard, Settings, Newsletter).
**Learning:** The lack of length limits could allow malicious users to submit excessively large payloads, potentially leading to resource exhaustion, local storage quota exhaustion, or client-side DoS.
**Prevention:** Always add appropriate maxLength attributes to all text and email inputs across the application along with security context comments.
