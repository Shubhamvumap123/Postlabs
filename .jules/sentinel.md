...
**Vulnerability:** User inputs and textareas lack length constraints, exposing the app to potential client-side DoS or state/storage abuse.
**Learning:** Relying purely on UI visual constraints without explicit DOM length limits fails to protect application state.
**Prevention:** Always set appropriate `maxLength` attributes on all `input` and `textarea` fields handling user input.
