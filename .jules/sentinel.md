...
## 2026-04-20 - Input Length Limits
**Vulnerability:** Missing input length limits on user-facing forms (DoS risk and unbounded state growth).
**Learning:** Unrestricted input fields can lead to excessively large data processing client-side or large state allocations, contributing to performance degredation or DoS.
**Prevention:** Always ensure all user-facing HTML `input` and `textarea` fields have appropriate `maxLength` attributes.
