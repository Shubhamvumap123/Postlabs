...
## 2026-04-18 - Text Animation Span Generation Bottleneck
**Learning:** Generating large arrays of span elements inside the render loop based on dynamic state causes unnecessary allocations and main thread blocking.
**Action:** Extract static text outside the component, memoize the span generation with `useMemo`, and toggle visibility using parent CSS attribute selectors (e.g., `group-data-[visible=true]`).
