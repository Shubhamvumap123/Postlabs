...
## 2026-04-19 - Optimize text animation rendering
**Learning:** Generating arrays of span elements inside the render loop based on dynamic state causes unnecessary re-renders and can block the main thread.
**Action:** Extract static text outside the component, memoize span generation with `useMemo`, and toggle visibility using parent CSS attribute selectors like `group-data-[visible=true]`.
