...
**Action:** Always eagerly load above-the-fold critical images (use `fetchPriority="high"` where appropriate) and explicitly apply `loading="lazy"` to all below-the-fold images. Never apply `loading="lazy"` to LCP elements.
## 2026-09-12 - Missing Compound Indexes in Frequent Queries
**Learning:** The dashboard's primary data fetch query filters by user ID and sorts by creation date. Without a compound index, this results in a full collection scan and a blocking in-memory sort in MongoDB, which degrades backend performance and API latency as the database grows.
**Action:** Always verify that frequent queries involving both filtering and sorting are supported by an appropriate compound index (e.g., `{ filterField: 1, sortField: -1 }`).
