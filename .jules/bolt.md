...
## 2026-09-19 - Extracted Search String Redundant Allocations
**Learning:** In list filtering operations (like filtering jobs by search query), calling string transform methods like `.toLowerCase()` on an invariant search string inside the `.filter()` loop forces the runtime to allocate redundant string objects and perform duplicate O(N) string processing operations, unnecessarily increasing CPU and memory load during fast typing.
**Action:** Always extract invariant computations (such as `.toLowerCase()` on search strings) outside of list iteration loops (e.g., inside the `useMemo` block but outside the `.filter` callback) to prevent redundant O(N) memory allocations and minimize CPU work.
