...
## 2024-10-24 - Framer Motion useScroll Optimization
**Learning:** Using independent DOM scroll event listeners (even if throttled with requestAnimationFrame) causes decentralized DOM reads that can lead to layout thrashing. framer-motion's useScroll centralizes and optimizes these reads.
**Action:** When optimizing components that track scroll position, prefer using framer-motion's useScroll and useMotionValueEvent hooks to leverage centralized read/write batching.
