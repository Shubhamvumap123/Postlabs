...
## 2024-05-18 - About Component Main Thread Blocking Bottleneck
**Learning:** Generating an array of ~200 span elements inside the render cycle of the `About` component using `text.split('').map(...)` on every state change caused a massive main thread stall, dropping frames when triggering the visibility state.
**Action:** Always extract static text constants and wrap the heavy span generation logic inside a `useMemo` block that runs exactly once. Furthermore, offload the visibility transition state directly to CSS using parent attribute selectors (`group-data-[visible=true]:opacity-100`) to entirely skip React's reconciliation phase during the animation.
