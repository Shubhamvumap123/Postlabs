...
## 2024-05-18 - Animated Spans React State Bottleneck
**Learning:** Generating large amounts of DOM elements (like splitting text into >100 `<span>` elements for animation) inside a component's render function, and then driving their individual animation states via a React state variable (like `isVisible`), causes severe main thread blocking and animation jank. React must unnecessarily diff and re-render every single DOM node whenever the visibility state changes.
**Action:** Explicitly memoize the generation loop using `useMemo` based on static data, and decouple the dynamic state entirely by relying on parent CSS attribute selectors (e.g., `group-data-[visible=true]`). This allows React to only update a single attribute on the parent, delegating the heavy lifting to the browser's CSS engine.
