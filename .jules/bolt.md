...
## 2026-09-20 - Array Filter O(N) String Computations
**Learning:** Invariant string operations (like `.toLowerCase()`) inside array `.filter()` loops cause redundant memory allocations and unnecessary computation for every item.
**Action:** Extract invariant string manipulations outside of list iteration loops (e.g., `.filter()`) into `useMemo` to prevent redundant O(N) memory allocations and minimize CPU work.
